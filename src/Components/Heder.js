import { NavLink } from "react-router-dom";

import {
  Navbar,
  Container,
  Nav,
  Button,
  NavDropdown,
  Dropdown,
} from "react-bootstrap";

function Heder() {
  let img = [
    "data:image/webp;base64,",
  ];

  return (
    <>
      <Navbar bg="light" expand="lg" className="">
        <Container>
          <Navbar.Brand as={NavLink} to={"/"}>
            <img src={img[0]} alt="brand img" />
          </Navbar.Brand>

          <div className="mainnav">
            <Navbar.Toggle
              aria-controls="basic-navbar-nav"
              className="navbar"
            />
            <Navbar.Collapse id="basic-navbar-nav">
              <Nav className="">
                <Nav.Link style={{ margin: "10px" }} as={NavLink} to={"/"}>
                  Home
                </Nav.Link>
                <Nav.Link style={{ margin: "10px" }} as={NavLink} to={"/about"}>
                  About
                </Nav.Link>
                <Nav.Link
                  style={{ margin: "10px" }}
                  as={NavLink}
                  to={"/services"}
                >
                  Services
                </Nav.Link>
                <Nav.Link
                  style={{ margin: "10px" }}
                  as={NavLink}
                  to={"/portfolio"}
                >
                  Portfolio
                </Nav.Link>

                <NavDropdown style={{ margin: "10px" }} title="Page">
                  <NavDropdown.Item href="#action/3.1">Bloge</NavDropdown.Item>
                  <NavDropdown.Item href="#action/3.2">
                    Blog Detalls
                  </NavDropdown.Item>
                  <NavDropdown.Item href="#action/3.3">
                    Element
                  </NavDropdown.Item>
                  <Dropdown.Item>Portfolio details</Dropdown.Item>
                </NavDropdown>

                <Nav.Link
                  style={{ margin: "10px" }}
                  as={NavLink}
                  to={"/contact"}
                >
                  Contact
                </Nav.Link>
                <div className="header-right-btn f-right d-none d-xl-block ml-20">
                  <Nav.Link>
                    <Button className="heder-btn">Get Free Consultent</Button>
                  </Nav.Link>
                </div>
              </Nav>
            </Navbar.Collapse>
          </div>
        </Container>
      </Navbar>
    </>
  );
}

export default Heder;
